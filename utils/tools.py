...
def get_variance_level(preprocess_config, model_config, data_loading=True):
    ...
    Outputs:
        ...
        pitch_feature_level, energy_feature_level: ["frame_level", "phoneme_level"]
            The feature_level in config where the models will learn each variance in this level regardless of the input level.
    """
...
def to_device(data, device):
    if len(data) == 26:
        ...
        cur_sen_dig_texts = torch.stack(cur_sen_dig_texts).to(device)
        cur_sen_dig_audios = torch.stack(cur_sen_dig_audios).to(device)
        cur_audio_sen_ans = torch.stack(cur_audio_sen_ans).to(device)

        cur_sen_dig_text_len = torch.from_numpy(cur_sen_dig_text_len).to(device)
        cur_sen_dig_audio_len = torch.from_numpy(cur_sen_dig_audio_len).to(device)


        for idx1 in range(len(cur_text_graphs)):
            if cur_text_graphs[idx1] != None:
                cur_text_graphs[idx1] = cur_text_graphs[idx1].to(device)

        for idx1 in range(len(cur_audio_graphs)):
            if cur_audio_graphs[idx1] != None:
                cur_audio_graphs[idx1] = cur_audio_graphs[idx1].to(device)


        for idx1 in  range(len(text_positive_graphs)):
            for idx2 in range(len(text_positive_graphs[idx1])):
                text_positive_graphs[idx1][idx2] = text_positive_graphs[idx1][idx2].to(device)

        for idx1 in  range(len(audio_positive_graphs)):
            for idx2 in range(len(audio_positive_graphs[idx1])):
                audio_positive_graphs[idx1][idx2] = audio_positive_graphs[idx1][idx2].to(device)

        for idx1 in  range(len(text_negative_graphs)):
            for idx2 in range(len(text_negative_graphs[idx1])):
                text_negative_graphs[idx1][idx2] = text_negative_graphs[idx1][idx2].to(device)

        for idx1 in  range(len(audio_negative_graphs)):
            for idx2 in range(len(audio_negative_graphs[idx1])):
                audio_negative_graphs[idx1][idx2] = audio_negative_graphs[idx1][idx2].to(device)
    ...
    if len(data) ==13:
        (ids, raw_texts, speakers, texts, src_lens, max_src_len, spker_embeds, emotions, text_posi_embs, audio_posi_embs, graph_texts, graph_audios, history_info) = data

        for i in range(len(text_posi_embs)):
            text_posi_embs[i] = text_posi_embs[i].to(device)
            audio_posi_embs[i] = audio_posi_embs[i].to(device)

        text_posi_embs = [emb.type(torch.float32) for emb in text_posi_embs]
        audio_posi_embs = [emb.type(torch.float32) for emb in audio_posi_embs]
        text_posi_embs = torch.stack(text_posi_embs)
        audio_posi_embs = torch.stack(audio_posi_embs)

        for idx1 in range(len(graph_texts)):
            if graph_texts[idx1] != None:
                graph_texts[idx1] = graph_texts[idx1].to(device)

        for idx1 in range(len(graph_audios)):
            if graph_audios[idx1] != None:
                graph_audios[idx1] = graph_audios[idx1].to(device)
        ...
        return (ids, raw_texts, speakers, texts, src_lens, max_src_len, spker_embeds, emotions, text_posi_embs, audio_posi_embs, graph_texts, graph_audios, history_info)
...
def log(
    ...
    if losses is not None:
        ...
        logger.add_scalar("Loss/an_loss", losses[8], step)
        logger.add_scalar("Loss/text_contr_loss", losses[9], step)
        logger.add_scalar("Loss/audio_contr_loss", losses[10], step)
...
# 用于生成前面padding的掩码
def get_mask_from_lengths_pad_left(lengths, max_len=None):
    batch_size = lengths.shape[0]
    if max_len is None:
        max_len = torch.max(lengths).item()
    ...
    mask = ids < (max_len - lengths.unsqueeze(1))
