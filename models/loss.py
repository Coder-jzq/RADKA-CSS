...
class CompTransTTSLoss(nn.Module):
    ...
    def __init__(self, preprocess_config, model_config, train_config):
        ...
        self.retri_text_conloss = ContrastiveLoss()
        self.retri_audio_conloss = ContrastiveLoss()


    def forward(self, inputs, predictions, other_output, device, step):
        (
            texts,
            _,
            _,
        ...
        an_loss = self.mse_loss(an_predict_emb, aN_ground_truth)
        total_loss += an_loss


        retri_text_loss = self.retri_text_conloss(posi_text_semantics_emb, nega_text_semantics_emb)
        total_loss += retri_text_loss





        retri_audio_loss = self.retri_audio_conloss(posi_audio_style_emb, nega_audio_style_emb)
        total_loss += retri_audio_loss
...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, posi_embs, nega_embs):
        batch_size, K, feature_dim = posi_embs.size()

        posi_sim = F.cosine_similarity(posi_embs.unsqueeze(2), posi_embs.unsqueeze(1), dim=-1)  # (B, K, K)

        nega_sim = F.cosine_similarity(posi_embs.unsqueeze(2), nega_embs.unsqueeze(1), dim=-1)  # (B, K, M)

        posi_exp = torch.exp(posi_sim / self.tao)  # (B, K, K)

        nega_exp = torch.exp(nega_sim / self.tao)  # (B, K, M)

        denominator_exp = torch.cat((posi_exp, nega_exp), dim=-1)  # (B, K, K + M)

        loss = -torch.log(posi_exp.sum(dim=-1) / denominator_exp.sum(dim=-1))  # (B, K)
